 ...
 * <h2>How it works</h2>
 * <ol>
 *   <li><strong>Locate</strong> – looks for {@code build.java} in the directory
 *       supplied to {@link #run(Path, Build, String)}.</li>
 *   <li><strong>Compile</strong> – invokes {@link javax.tools.JavaCompiler}
 *       in-process (no external {@code javac} process), forwarding the current
 *       JVM classpath so the script can import {@code dev.haruki7049.buildente.*}.</li>
 *   <li><strong>Load</strong> – loads the compiled {@code build} class from the
 *       temporary output directory via {@link URLClassLoader}.</li>
 *   <li><strong>Execute</strong> – casts the class to {@link BuildScript}, calls
 *       {@link BuildScript#build(Build)}, then runs the requested step.</li>
 * </ol>
 *
 * <h2>Limitations (POC)</h2>
 * <ul>
 *   <li>The script class must be named exactly {@code build} (lowercase).</li>
 *   <li>Only a single source file is supported in this proof-of-concept.</li>
 *   <li>Requires a JDK (not just a JRE) at runtime for {@code javax.tools}.</li>
 * </ul>
 */
public final class ScriptRunner {
     ...
     *
     * <p>This method encapsulates the full lifecycle: locate → compile → load →
    ...
    /**
     * Compiles {@code scriptFile} using the in-process Java compiler.
     * The current JVM classpath is forwarded so the script can reference
     * Buildente library classes.
     *
    ...
    private static Path compile(Path scriptFile) {
        ...
        try (var fileManager = compiler.getStandardFileManager(diagnostics, null, null)) {
            ...
            if (!success) {
                throw new BuildScriptException(
                    "Script compilation failed. See diagnostics above."
                );
            }
    ...
    /**
     * Loads the compiled {@code build} class from {@code outputDir} and
     * instantiates it as a {@link BuildScript}.
     *
        ...
        try {
            ...
            try (URLClassLoader loader = new URLClassLoader(urls, parent)) {
                ...
                if (!BuildScript.class.isAssignableFrom(scriptClass)) {
                    throw new BuildScriptException(
                        "Class '" + SCRIPT_CLASS + "' in build.java must implement "
