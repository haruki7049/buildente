 ...
 *
 * <p>Corresponds to {@code b.addExecutable(name, source)} in the Buildente API, mirroring {@code
...
public class Executable extends Step {
  ...
  /**
   * The original logical name of this executable (e.g. {@code "HelloWorld"}). This is the
   * fully-qualified class name used by {@link RunStep} to invoke the program.
   */
  ...
  /**
   * Returns the logical name (class name) of this executable. Used by {@link RunStep} to construct
   * the {@code java} command.
   *
  ...
  /**
   * Invokes {@code javac} via {@link ProcessBuilder} to compile {@link #sourceFile}. Output is
   * placed in {@value #OUTPUT_DIR}.
   *
  ...
  protected void execute() {
    ...
    try {
      ProcessBuilder pb = new ProcessBuilder("javac", "-d", OUTPUT_DIR, sourceFile);
      pb.inheritIO();
      ...
      if (exitCode != 0) {
        throw new RuntimeException(
            "[buildente] javac exited with code " + exitCode + " for source: " + sourceFile);
      }
