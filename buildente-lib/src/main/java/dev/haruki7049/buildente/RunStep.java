 ...
 *
 * <p>Corresponds to {@code b.addRunArtifact(exe)} in the Buildente API,
 * mirroring {@code std.Build.addRunArtifact} from the Zig build system.
 *
 * <p>This step automatically depends on the given {@link Executable}, so the
 * source will always be compiled before it is run.
 */
public class RunStep extends Step {
    ...
    /**
     * Creates a run step for the given executable.
     * Automatically adds {@code executable} as a dependency.
     *
     ...
     * The classpath is set to {@link Executable#OUTPUT_DIR}.
     *
        ...
        try {
            ...
            if (exitCode != 0) {
                throw new RuntimeException(
                    "[buildente] java exited with code " + exitCode
                    + " for class: " + className
                );
            }
