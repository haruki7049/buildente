 ...
 * NOT compiled by Gradle ahead of time — instead, the Buildente engine (ScriptRunner) compiles and
 * loads it dynamically at runtime using {@code javax.tools.JavaCompiler} and {@code
 * URLClassLoader}.
 *
 * <p>Rules: - The file must be named exactly "build.java". - The public class inside must be named
 * exactly "build" (lowercase). - The class must implement {@link BuildScript}. - Only the {@code
 * build(Build b)} method needs to be implemented.
 */
public class Buildente implements BuildScript {
  ...
  public void build(Build b) {
    ...
    Executable exe = b.addExecutable("com.example.Hello", "src/Hello.java");
